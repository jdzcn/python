...
class Ui_MainWindow(object):
    ...
    def toolbtnpressed(self,a):
        if a.text()=='Server':
            text,ok=QtWidgets.QInputDialog.getMultiLineText(None,'Myssh','Your Server')
            if ok and text!='':
                print(text)
        if a.text()=='Sftp':
            seldir = QtWidgets.QFileDialog.getExistingDirectory(None, '上传文件')
        if a.text()=='Go':
            self.thread=Worker(None,self.comboBox.currentText())
...
class Worker(QThread):
    ...
    def run(self):
        ...
        ssh.connect('',port=28147, username='root', password='')
